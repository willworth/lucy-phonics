#!/bin/bash
# Generate remaining images one at a time with 5s delay between each
# Avoids OOM by not parallelizing
...
log() {
  echo "[$(date '+%H:%M:%S')] $1"
  echo "- $(date '+%H:%M:%S') — $1" >> "$WORKLOG"
}

# Init worklog
echo "## Image Generation Run — $(date '+%Y-%m-%d %H:%M')" >> "$WORKLOG"
echo "" >> "$WORKLOG"
...
" 2>&1)
  ...
  if [[ "$RESULT" == OK* ]]; then
    ...
    log "✅ $dir/$name.png — $RESULT (total: $GENERATED generated)"
  else
    ERRORS=$((ERRORS + 1))
    log "❌ $dir/$name.png — $RESULT"
  fi
...
log "🏁 COMPLETE — Generated: $GENERATED | Skipped: $SKIPPED | Errors: $ERRORS"
echo "" >> "$WORKLOG"

TOTAL=$(find assets/img -name "*.png" | wc -l)
echo "Total images on disk: $TOTAL"
log "Total images on disk: $TOTAL"

# Git commit and push
echo ""
echo "Committing to git..."
git add -A
git commit -m "feat: generate Phase 1 images ($GENERATED new, $TOTAL total)"
git push
log "Git pushed — $GENERATED new images committed"
